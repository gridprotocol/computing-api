...
func (ds *DockerService) ListImages() (map[string]string, error) {
	...
	if err != nil {
		logger.Errorf("Unable to list image, error: %+v", err)
	}
	...
	for _, image := range imageList {
		if len(image.RepoTags) == 0 || strings.EqualFold(image.RepoTags[0], "<none>:<none>") {
			if err := ds.RemoveImage(image.ID); err != nil {
				logger.Errorf("Failed delete unused image, error: %+v", err)
				continue
			}
...
func (ds *DockerService) CleanResource() {
	...
	if err != nil {
		logger.Errorf("Failed delete dangling image, error: %+v", err)
		return
	}

	if _, err = ds.c.ContainersPrune(ctx, filters.NewArgs()); err != nil {
		logger.Errorf("Failed delete unused container, error: %+v", err)
		return
	}

	images, err := ds.c.ImageList(context.Background(), types.ImageListOptions{})
	if err != nil {
		logger.Errorf("Failed get image list, error: %+v", err)
		return
	}

	for _, image := range images {
		if image.Containers == 0 {
			logger.Infof("start clean unused image, imageId: %s", image.ID)
